 ...
 *
 *  ATTENTION: MODIFY THE GLOBAL CONSTANTS BEFORE RUNNING:
 *  KK, polind, fa_axion, m_ax, R_inf	
 *		 
 */
...
extern long double var[5][num];
extern long double k1[5], k2[5], k3[5], k4[5];                                                                             

extern long double eps_ns, grr;                          // eps_ns=pow(KK,1/polind)*pow(P,(polind-1)/polind)/(polind-1)
                                                               // grr is the r,r component of the metric tensor gmunu
                                                               // mass is the final mass of the fermion-axion model
                       
extern long double Rmax, Cutphi, CutP, omega2, delta_r, r;     // variable for the ODE and the RK4 solver
