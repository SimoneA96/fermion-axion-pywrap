...
long double var[5][num];
long double k1[5], k2[5], k3[5], k4[5];
                                                                               

long double eps_ns, grr;                          // eps_ns=pow(KK,1/polind)*pow(P,(polind-1)/polind)/(polind-1)
                                                        // grr is the r,r component of the metric tensor gmunu
                                                        // mass is the final mass of the fermion-axion model
                                                        

long double Rmax, omega2, Cutphi, CutP, delta_r, r;     // variable for the ODE and the RK4 solver
...
long double V(long double phi) {
  return 2.0 * pow(m_ax,2) * pow(fa_axion,2) / 0.22 * (1.0 - sqrt(1.0 - 4 * 0.22 * pow(sin( phi/(2.0*fa_axion)),2)));
}
long double dV(long double phi) {
  if (phi!=0.0){
    return 2.0 * pow(m_ax,2)  * fa_axion * sin(phi/(2.0*fa_axion)) * cos(phi/(2.0*fa_axion)) /
...
int rhs(long double r,  long double omega2, long double h) {
  ...
  //OSS: eps_ns=K*rho --> eps_ns=10*sqrt(P) eps_ns = pow(KK,1/polind)*pow(P,(polind-1)/polind)/(polind-1)
  eps_ns = pow(KK,1/polind)*pow(P,(polind-1)/polind)/(polind-1);
  rho_tmp = pow(P/KK,1/polind);

  F[0] = 0.5*a*((1-a*a)/r + 8*Pi*r*( pow(dphi,2) + omega2 * pow(a*phi/alp, 2) + a*a * V(phi)+a*a*rho_tmp*(1+eps_ns)));
  F[1] = 0.5*alp*((a*a-1)/r + 8*Pi*r*( pow(dphi,2) + omega2 * pow(a*phi/alp, 2) - a*a * V(phi)+a*a*P));
  F[2] = dphi;
  F[3] = -(1+a*a-8*Pi*pow(r*a,2)*(V(phi)+0.5*(rho_tmp*(1+eps_ns)-P)))*dphi/r+(dV(phi)-omega2/pow(alp,2))* a*a * phi;
  F[4] =-(rho_tmp*(1+eps_ns)+P)*0.5*((a*a-1)/r + 8*Pi*r*( pow(dphi,2) + omega2 * pow(a*phi/alp, 2) - a*a * V(phi)+a*a*P));
...
long double shooting_omega(long double rho0c, long double phi0c){
  ...
  while((omega_2-omega_1)/omega_1>2*order) {
    ...
    //cout << mid_omega << "  ";
    f_mid=ode_int(phi0c, rho0c, mid_omega, &grr);
