...
//const long double Pi=3.14159265358979323846L, KK=100.0, lambda= 0.0*4*Pi, m=1.0, fa_axion = 0.02, polind=2;
const long double Pi=3.14159265358979323846L;
const long double |KK=VALUE|, |lambda=VALUE|, |m=VALUE|, |fa_axion=VALUE|, |polind=VALUE|;
long double eps;
...
int func(long double r,  long double h) {
	...
	long double rho_tmp;
        ...
        F[0] = 0.5*a*((1-a*a)/r + 8*Pi*r*( pow(dphi,2) + omega2 * pow(a*phi/alp, 2) + a*a * V(phi)+a*a*0.1*sqrt(P)*(1+eps)));
        F[1] = 0.5*alp*((a*a-1)/r + 8*Pi*r*( pow(dphi,2) + omega2 * pow(a*phi/alp, 2) - a*a * V(phi)+a*a*P));
        F[2] = dphi;
        F[3] = -(1+a*a-8*Pi*pow(r*a,2)*(V(phi)+0.5*(0.1*sqrt(P)*(1+eps)-P)))*dphi/r+(dV(phi)-omega2/pow(alp,2))* a*a * phi;
        F[4] =-(0.1*sqrt(P)*(1+eps)+P)*0.5*((a*a-1)/r + 8*Pi*r*( pow(dphi,2) + omega2 * pow(a*phi/alp, 2) - a*a * V(phi)+a*a*P));
...
//rho_HD = (rho*(1+eps) + P) - P = rho + rho*eps = pow(P/K,1/polind) + P/(polind-1)
		...
		solfile5.open(folder + "/solrhobosoniso"+tmp+".txt");
	        ...
	        evolutionfile.open(folder + "/Mixed_"+phi_str +"_"+ rho_str +"_"+ lam_str+".dat");
		...
		existence_plot<<var[2][0]<<" "<<omega<<" "<<((R)/2)* (1-1/pow(var[0][num-1],2))<<endl;
		rfile << endl;
		solfile << endl;
		solfile1 << endl;
		solfile2 << endl;
